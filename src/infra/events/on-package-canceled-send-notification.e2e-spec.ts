...
import { cancelPackage, registerPackage } from 'test/e2e/package-flow';
...
describe('On Package Canceled (E2E)', () => {
  ...
  it('should send a notification when a package is canceled', async () => {
    ...
    const response = await cancelPackage(app, adminToken, packageId, {
      description: 'Order canceled by recipient',
    });
      ...
      const [notification] = await drizzleService.db
        ...
        .where(
          and(
            ...
            eq(notifications.title, 'Order canceled by recipient')
          )
        );

      expect(notification).toBeDefined();
      expect(notification.content).toBe(
        `Your Package was canceled: Eletronico..., the package code is: ${packageCode}`
      );
    });
  });
});
