...
describe('On Package Was Delivered (E2E)', () => {
  ...
  it('should send a notification when a package is delivered', async () => {
    ...
    const dpEmail = `delivered-evt-${Date.now()}@example.com`;
    const {
      response: dpRes,
      cpf: dpCpf,
      password: dpPassword,
    ...
    const [att] = await drizzleService.db
      ...
      .values({
        title: 'proof.jpg',
        key: `delivered-evt-${Date.now()}.jpg`,
      })
      .returning({ id: attachments.id });

    const response = await deliverPackage(app, dpToken, packageId, att.id, {
      ...
      const [notification] = await drizzleService.db
        ...
        .where(
          and(
            ...
            eq(notifications.title, 'Delivered to door')
          )
        );

      expect(notification).toBeDefined();
      expect(notification.content).toBe(
        `Your package Sneakers has been delivered. Package code: ${packageCode}`
      );
    });
  });
});
