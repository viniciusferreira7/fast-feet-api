...
describe('On Package Assigned To A Delivery Person (E2E)', () => {
  ...
  it('should send a notification when a package is assigned to a delivery person', async () => {
      ...
      const [notification] = await drizzleService.db
        ...
        .where(
          and(
            ...
            eq(notifications.title, 'Custom assignment note')
          )
        );

      expect(notification).toBeDefined();
      expect(notification.content).toBe(
        `Delivery person was assigned to get a package: Notebook D..., the package code is: ${packageCode}`
      );
    });
  });
});
