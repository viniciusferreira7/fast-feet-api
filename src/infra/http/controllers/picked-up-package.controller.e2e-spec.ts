...
describe('Picked Up Package (E2E)', () => {
  ...
  it('[PATCH] /packages/:packageId/pick-up — should return 200 when delivery person picks up their assigned package', async () => {
    ...
    // Register delivery person and get their DB id + authenticate them in one coherent flow
    const dpEmail = `pickup-dp-${Date.now()}@example.com`;
    const {
      response: dpRes,
      cpf: dpCpf,
      password: dpPassword,
  ...
  it('[PATCH] /packages/:packageId/pick-up — should return 400 when package is in pending status', async () => {
    const { response: recipientRes } = await registerRecipientPerson(
      app,
      apiToken
    );
    ...
    // Package stays in pending status — cannot be picked up
    const response = await pickUpPackage(app, deliveryToken, packageId);

    expect(response.statusCode).toBe(400);
  });

  it('[PATCH] /packages/:packageId/pick-up — should return 400 when package has no assigned delivery person', async () => {
    const { response: recipientRes } = await registerRecipientPerson(
      app,
      apiToken
    );
    ...
    // Seed awaiting_pickup but no deliveryPersonId set
    await drizzleService.db
      .update(packages)
      .set({ status: 'awaiting_pickup', deliveryPersonId: null })
      .where(eq(packages.id, packageId));
  ...
  it('[PATCH] /packages/:packageId/pick-up — should return 404 when package does not exist', async () => {
    const { accessToken: deliveryToken } =
      await createAuthenticatedDeliveryPerson(
        app,
        drizzleService,
    ...
    const response = await pickUpPackage(app, deliveryToken, randomUUID());

    expect(response.statusCode).toBe(404);
  });

  it('[PATCH] /packages/:packageId/pick-up — should return 403 when requester is an admin', async () => {
    const response = await request(app.getHttpServer())
      .patch(`/packages/${randomUUID()}/pick-up`)
  ...
  it('[PATCH] /packages/:packageId/pick-up — should return 401 when no token is provided', async () => {
    const response = await request(app.getHttpServer()).patch(
      `/packages/${randomUUID()}/pick-up`
    );
