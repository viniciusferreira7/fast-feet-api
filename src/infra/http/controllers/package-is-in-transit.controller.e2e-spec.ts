...
describe('Package Is In Transit (E2E)', () => {
  ...
  it('[PATCH] /packages/:packageId/in-transit — should return 200 when admin marks package as in transit', async () => {
    ...
    await drizzleService.db
      .update(packages)
      .set({ status: 'at_distribution_center', deliveryPersonId })
  ...
  it('[PATCH] /packages/:packageId/in-transit — should return 400 when package is in pending status', async () => {
    const { response: recipientRes } = await registerRecipientPerson(
      app,
      apiToken
    );
    ...
    // Package stays in pending — cannot transition to in_transit
    const response = await markPackageInTransit(
      app,
      adminToken,
      packageId,
  ...
  it('[PATCH] /packages/:packageId/in-transit — should return 404 when package does not exist', async () => {
    const { response: dpRes } = await registerDeliveryPerson(app, adminToken);
    const deliveryPersonId = dpRes.body.delivery_person.id as string;
  ...
  it('[PATCH] /packages/:packageId/in-transit — should return 404 when delivery person does not exist', async () => {
    const { response: recipientRes } = await registerRecipientPerson(
      app,
      apiToken
    );
    ...
    await drizzleService.db
      .update(packages)
      .set({ status: 'at_distribution_center' })
      .where(eq(packages.id, packageId));
  ...
  it('[PATCH] /packages/:packageId/in-transit — should return 403 when requester is a delivery person', async () => {
    ...
    const response = await request(app.getHttpServer())
      .patch(`/packages/${randomUUID()}/in-transit`)
  ...
  it('[PATCH] /packages/:packageId/in-transit — should return 401 when no token is provided', async () => {
    const response = await request(app.getHttpServer()).patch(
      `/packages/${randomUUID()}/in-transit`
    );
