...
import { dropOffPackage, registerPackage } from 'test/e2e/package-flow';
...
describe('Drop Off Package At Distribution Center (E2E)', () => {
  ...
  it('[PATCH] /packages/:packageId/drop-off — should return 200 when delivery person drops off their picked-up package', async () => {
    ...
    const dpEmail = `dropoff-dp-${Date.now()}@example.com`;
    const {
      response: dpRes,
      cpf: dpCpf,
      password: dpPassword,
    ...
    await drizzleService.db
      .update(packages)
      .set({ status: 'picked_up', deliveryPersonId: dpId })
      .where(eq(packages.id, packageId));

    const response = await dropOffPackage(app, dpToken, packageId);

    expect(response.statusCode).toBe(200);
    expect(response.body.package).toEqual(
      expect.objectContaining({
        id: packageId,
        status: 'at_distribution_center',
      })
    );
  });

  it('[PATCH] /packages/:packageId/drop-off — should return 400 when package is in pending status', async () => {
    const { response: recipientRes } = await registerRecipientPerson(
      app,
      apiToken
    );
    ...
    // Package stays in pending — cannot drop off
    const response = await dropOffPackage(app, deliveryToken, packageId);

    expect(response.statusCode).toBe(400);
  });

  it('[PATCH] /packages/:packageId/drop-off — should return 404 when package does not exist', async () => {
    const { accessToken: deliveryToken } =
      await createAuthenticatedDeliveryPerson(
        app,
        drizzleService,
    ...
    const response = await dropOffPackage(app, deliveryToken, randomUUID());

    expect(response.statusCode).toBe(404);
  });

  it('[PATCH] /packages/:packageId/drop-off — should return 403 when requester is an admin', async () => {
    const response = await request(app.getHttpServer())
      .patch(`/packages/${randomUUID()}/drop-off`)
  ...
  it('[PATCH] /packages/:packageId/drop-off — should return 401 when no token is provided', async () => {
    const response = await request(app.getHttpServer()).patch(
      `/packages/${randomUUID()}/drop-off`
    );
